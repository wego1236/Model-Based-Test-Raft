...
func Make_config(n int, unreliable bool, snapshot bool) *config {
	ncpu_once.Do(func() {
		if runtime.NumCPU() < 2 {
...
func (cfg *config) checkLogs(i int, m raft.ApplyMsg) (string, bool) {
	err_msg := ""
	v := m.Command
...
// contents
func (cfg *config) applier(i int, applyCh chan raft.ApplyMsg) {
	for m := range applyCh {
...
func (cfg *config) applierSnap(i int, applyCh chan raft.ApplyMsg) {
	lastApplied := 0
	for m := range applyCh {
		if m.SnapshotValid {
...
//
func (cfg *config) start1(i int, applier func(int, chan raft.ApplyMsg)) {
	cfg.crash1(i)
...
func (cfg *config) appendAll(server int) {
	if _, isLeader := cfg.rafts[server].GetState(); !isLeader {
		//DPrintf("%d, is not leader, don't send entries", rf.me)
		return
	}
	cfg.rafts[server].Replicate()
}
