        ...
        ELSE LET entries == GetEntriesToIdxBaseHelper(log'[i], commitIdx'[i],
                 ...
                 F[j \in 0..nServer] ==
                    ...
                    ELSE LET k == CHOOSE k \in Server: k \notin F[j-1][1]
                             logLenOk == LastIdxNext(k) >= commitIdx'[i]
...
MonotonicCurrentTerm == \A i \in Server: currentTerm' [i] >= currentTerm[i]
...
InvHelper(pc) == <<AtMostOneLeaderPerTerm,
                   CommittedLogReplicatedMajority,
                   CommittedLogDurable(pc),
                   FollowerLogLELeaderLogAfterAE(pc),
...
NetworkPartition ==
    ...
    IN IF n = 0
       ...
       ELSE /\ UNCHANGED noScrVars
            /\ ScrNotPartitioned
            /\ ScrNotLocked
            /\ ScrSet(ScrSetPartNodes(n), <<"NetworkPartition">>)
...
\* Leader sends snapshot to other servers
SendSnapshotAllHelper(i) ==
    IF IsLeader(i)
    THEN LET except == { j \in Server:
                            ~NeedSnapshot(snapshotLastIdx'[i], nextIdx[i][j]) }
         IN LockArgs(i, except, SnapshotRequest)
    ELSE ScrUnsetLock

SendSnapshot(i, j) ==
    /\ i /= j
    /\ IF NeedSnapshot(snapshotLastIdx[i], nextIdx[i][j])
...
AppendEntriesHelper(i, j) ==
    IF NeedSnapshot(snapshotLastIdx'[i], nextIdx'[i][j])
    THEN SendSnapshotHelper(i, j) ELSE
    LET entries == IF IsNullEntryIdx(i, nextIdx'[i][j])
                   THEN <<>>
                   ELSE GetEntriesForAppend(i, nextIdx'[i][j])
        prevEty == GetEntry(i, nextIdx'[i][j] - 1)
          ...
          ELSE IF currentTerm[i] < b.term
               ...
               ELSE /\ votesGranted' = [ votesGranted EXCEPT ![i] =
                    ...
                    /\ IF IsQuorum(votesGranted'[i])
                       THEN /\ UNCHANGED <<currentTerm, votedFor, logVars,
                                           snapshotVars>>
                            /\ BecomeLeader(i)
                            \* Note: sends append entries immediately
                            /\ ScrSet(LockArgs(i, {}, AppendEntriesRequest) @@
                                      ScrIncRecvUnorder(m),
...
ExecSnapshot(i) ==
    /\ UNCHANGED <<serverVars, candidateVars, leaderVars,
                   commitIdx, networkVars>>
    /\ BeginSnapshot(i)
    /\ EndSnapshot(i)
    /\ IF IsLeader(i)
       THEN ScrSet(SendSnapshotAllHelper(i) @@ ScrIncSnapshot, 
                    <<"ExecSnapshot: leader", i, commitIdx[i]>>)
       ELSE ScrSet(ScrIncSnapshot,
...
DoLoop ==
    ...
    /\ LET a  == CHOOSE x \in scr.lock.args: TRUE
           ...
           i  == a.from
           j  == a.to
           ok == ScrNodeConnected(i, j)
       IN CASE t = AppendEntriesRequest ->
                    /\ AppendEntries(i, j)
                    /\ ScrSet(s, <<"DoLoop: AppendEntries",
                                   i, j, scr.nSent + 1, ok>>)
            [] t = RequestVoteRequest   ->
                    /\ RequestVote(i, j)
                    /\ ScrSet(s, <<"DoLoop: RequestVote",
                                   i, j, scr.nSent + 1, ok>>)
            [] t = SnapshotRequest      ->
                    /\ SendSnapshot (i, j)
                    /\ ScrSet(s, <<"DoLoop: SendSnapshot",
                                   i, j, scr.nSent + 1, ok>>)
...
Timeout(i) == /\ ~IsLeader(i)
              /\ IF Cardinality(Server) > 1
                 THEN /\ UNCHANGED <<networkVars, leaderVars, logVars,
                                     snapshotVars>>
                      /\ BecomeCandidate(i)
...
ClientRequest(i, v) ==
    ...
    /\ IF Cardinality(Server) = 1
       THEN /\ commitIdx' = [ commitIdx EXCEPT ![i] = @ + 1 ]
            /\ UNCHANGED <<serverVars, candidateVars, leaderVars, networkVars,
                           snapshotVars>>
       ELSE UNCHANGED <<noLogVars, networkVars>>
    /\ AppendEntriesExceptBehindIdx(i, LastIdxNext(i))
...
DoHandleSnapshotRequest         == ScrNotLocked /\ \E m \in messages:
...
DoDuplicate                     == ScrNotLocked /\ \E m \in messages :
                                    Duplicate(m)
DoSnapshot                      == ScrNotLocked /\ \E i \in Server:
                                    ExecSnapshot(i)
