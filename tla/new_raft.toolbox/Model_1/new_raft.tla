...
MonotonicMatchIdx ==
    \A i \in Server:
        IF state[i] = Leader
        THEN  \A j \in Server:
            LET next_mactchIdx == matchIndex'[i][j]
                cur_matchIdx == matchIndex[i][j] 
            IN    
                \/ next_mactchIdx >= matchIndex[i][j]
                \/ pc = "Restart" /\ next_mactchIdx = 0
        ELSE TRUE
