       ...
       ELSE IF stale_msg
            THEN /\ UNCHANGED <<candidateVars, leaderVars, logVars, serverVars, scr>>
                 ...
                 /\ pc' = <<"HandleRequestVoteRequest: stale msg",
                                i, j, m.seq>>
            ELSE /\ state'       = [ state       EXCEPT ![i] = IF demote THEN Follower ELSE @]
                 /\ currentTerm' = [ currentTerm EXCEPT ![i] = IF demote THEN body.term ELSE @]
                 /\ votedFor' = [ votedFor  EXCEPT ![i] = IF demote THEN Nil ELSE @] 
                 /\ UNCHANGED <<leaderVars, logVars, candidateVars>>
                 /\ Reply(msg, m)
                 /\ ScrSet(ScrIncSent)
                 /\ IF canGrant
                    THEN  
                    ...
                    ELSE 
                        ...
                        /\ pc' = <<"HandleRequestVoteRequest: not granted",
                                   i, j, m.seq>>
