...
HandleAppendEntriesRequest(i, j, m) ==
    ...
    IN  /\  IF b.mterm < currentTerm[i] \* Term is smaller, reply false.
            ...
            ELSE /\ BecomeFollower(i ,b.term) \* here serverVars has changed
                 /\ IF b.prevLogIdx > Len(log[i])
                    THEN /\ UNCHANGED <<logVars>>
                         /\ Reply(msg @@ ("body" :> rb1), m)
                         /\ pc = <<"HandleAppendEntriesRequest: would leave gap",
                          i, j>>
                    ELSE IF /\ ~log[b.prevLogIndex].term = b.prevLogTerm
                         THEN \* log mismatch  
                                /\ UNCHANGED <<logVars>>
                         ...
                         ELSE /\ IF Len(b.entries) = 0
                                 ...
                                 ELSE LET new == [index2 \in 1..b.prevLogIdx |-> \*todo may have bugs
                                                        log[i][index2]]  \* prevlogIndex -> end change to b.entries
                                          new1 == Append(new, b.entries)
                              ...
                              /\ pc = <<"HandleAppendEntriesRequest: success",
                                       i, j>>
        /\ UNCHANGED <<candidateVars, leaderVars, scr>>

\* Advance commmitIdx.
AdvanceCommitIdx(s, idx) ==
    LET c1 == /\ commitIndex[s] < idx 
              /\ /\ Len(log[s]) >= idx
                 /\ log[s].term = currentTerm[s]
...
HandleAppendEntriesResponse(i, j, m) == 
    ...
    IN IF not_leader
       THEN /\ UNCHANGED <<serverVars, candidateVars, logVars, leaderVars>>
            /\ Discard(m)
            /\ pc = <<"HandleAppendEntriesResponse: not leader", i , j>>
       ELSE IF demote
            ...
            ELSE IF stale_msg
                 ...
                 ELSE IF retry
                      THEN /\ UNCHANGED <<serverVars, candidateVars, matchIndex,
                           ...
                           /\ pc = <<"HandleAppendEntriesResponse: mismatch and retry", i, j>>
...
HandleRequestVoteRequest(i, j, m) ==
    LET body        == m.body
        logOk       ==  \/ body.lastLogIdx > LastTerm(log[i])
                        \/ /\ body.lastLogTerm = LastTerm(log[i])
                           /\ body.lastLogIdx >= Len(log[i])
        canGrant    == /\ body.term >= currentTerm[i]
                       /\ logOk
                       /\ votedFor[i] \in {Nil, j}
        ...
        msg         == [ type |-> RequestVoteResponse,
                         ...
                         body |-> rb ]
    IN IF IsLeader(i)
       THEN /\ UNCHANGED <<serverVars, leaderVars, logVars, InitCandidateVars>>
            /\ Reply(msg, m)
            /\ pc = <<"HandleRequestVoteRequest: leader not granted",
                              i, j>>
       ELSE IF stale_msg
            THEN /\ UNCHANGED <<candidateVars, leaderVars, logVars, serverVars>>
                 /\ Discard(m)
...
Timeout(i) == /\ ~IsLeader(i)
              /\ UNCHANGED <<leaderVars, logVars, voterLog, votesResponded>>
              /\ BecomeCandidate(i)
...
Receive(m) ==
    LET t == m.type
        ...
        j == m.from
    IN  /\ UNCHANGED <<scr>>
        /\ CASE t = AppendEntriesRequest  -> HandleAppendEntriesRequest (i, j, m)
...
DuplicateMessage(m) ==
    /\ Send(m)
    /\ pc' = <<"NetDuplicate",m>>
    /\ UNCHANGED <<serverVars, candidateVars, leaderVars, logVars, scr>>
